...
class GanttChart:
    ...
    def main_gantt(cls):
        ...
        fig6.update_layout(
            yaxis=dict(categoryorder='array', tickfont=dict(size=24), categoryarray=categories)
        )
        fig6.show()
        fig6.write_html(
            f"{pathConfig.simulator_result_gantt_chart_path}{os.sep}{Parameters.simulation_time}_{cls.dataset_id}_gantt.html")
    ...
    @classmethod
    def modify_pattern(cls, bar, shape):
        """
        막대의 너비를 설정합니다.
        width = (단위 px)
        """
        bar.marker.pattern['shape'] = shape
    ...
    @classmethod
    def gantt_chart_for_labeling(cls):
        """
        ...
        [(cls.modify_pattern(bar, "/")) for bar in fig_csv.data if ('index_' in bar.legendgroup)]
        return fig_csv